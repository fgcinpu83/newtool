...
export class ChromeController {
    ...
    async launchChrome(@Body() body: LaunchChromeDto) {
        console.log('[API] Launch Chrome body:', body);
    ...
    async getStatus() {
        try {
            const statusA = await isChromeLaunched('A');
            const statusB = await isChromeLaunched('B');
            const tabsA = statusA ? await getAccountTabs('A') : [];
            const tabsB = statusB ? await getAccountTabs('B') : [];
