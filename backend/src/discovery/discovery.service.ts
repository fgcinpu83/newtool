...
interface MatchEntry {
    matchId: string;
    globalId?: string; // 🛡️ v8.0: Master ID mapping for paired matches
    provider: 'A' | 'B';
    home: string;
    away: string;
    league: string;
...
export class DiscoveryService implements OnModuleInit {
    ...
    private bypassAllFilters = false; // 🛡️ v7.4.3: REACTIVE FILTERS (Adjusted for real football)

    // 🛡️ v7.4 OIPM (Odds Ingestion Per Minute)
    private oipmCounts = { A: 0, B: 0 };
    private wireLog = path.join(process.cwd(), 'logs', 'wire_debug.log');

    // 🛡️ v4.0 AUTO-REFRESH STATE
    ...
    // 🛡️ v5.2 REGISTRY WARM-UP - Pre-allocate for large data
    private registryBWarmup = false;

    // 🛡️ v3.7 LOCAL ORDER BOOK (LOB) CONFIG
    private readonly LOB_TTL_MS = 120000; // 🛡️ v8.8: Safer TTL for EVENT_DRIVEN providers (AFB88)
    ...
    onModuleInit() {
        ...
        // 🚀 HEARTBEAT CLEANUP (Logika Pendeteksian Ghost Match)
        setInterval(() => this.cleanRegistry(), this.CLEANUP_INTERVAL_MS);

        this.logger.log(`[LOB-ENGINE] ✅ Heartbeat Cleanup Enabled (TTL: ${this.LOB_TTL_MS / 1000}s)`);
        setInterval(() => this.printStabilityReport(), 2000);

        // 🛡️ v7.5 Flow Transparency Trace (Every 10 seconds)
        setInterval(() => {
        ...
        // 🔥 v3.5.7 Socket Pipe Fix: Broadcast raw MATCH_LIST every 1s
        ...
        // 🛡️ v7.4 OIPM Proof of Flow (Every 60 seconds)
        setInterval(() => {
            if (this.oipmCounts.A > 0 || this.oipmCounts.B > 0) {
                const oipmLog = `[OIPM] 🛡️ Aliran Data Mentah: A=${this.oipmCounts.A} pkts/min | B=${this.oipmCounts.B} pkts/min`;
                console.log(`%c${oipmLog}`, 'background:#004d40;color:#fff;font-weight:bold');
                this.logToDisk(oipmLog);
                // 🛡️ v7.5 Dashboard Sync
    ...
    public clearAllMemory() {
        this.logger.warn('🚨 [DISCOVERY] Wiping all registries (EMERGENCY BUFFER FLUSH)');
    ...
    public flushRegistryB() {
        this.logger.warn('🚨 [DISCOVERY] Flushing Registry B (Garbage Purge)');
        this.registryB.clear();
        ...
        this.gateway.sendUpdate('system_log', { level: 'info', message: '🧹 Registry B Flushed: Garbage data purged.' });
    }
    ...
    private warmupRegistryB() {
        this.logger.log('🔥 [WARM-UP] Preparing Registry B for large data (500+ events)');
        this.registryBWarmup = true;
        this.bypassAllFilters = false; // 🛡️ v7.6: Do NOT bypass filters during warm-up!

        // Disable stale data protection for B temporarily
        this.gateway.sendUpdate('system_log', {
            level: 'info',
            message: '🔥 Registry B WARM-UP: Ready to receive 500+ events. Stale protection DISABLED.'
        });

        this.logger.log('✅ [WARM-UP] Registry B is now ready for heavy data load');
    }

    /**
     * 🛡️ v5.2 SCANNER PRIORITY - Shift CPU to arbitrage engine when B > 20
     */
    private checkScannerPriority(countB: number) {
        ...
        if (countB >= 20 && !this.scannerPriorityActive) {
            ...
            this.logger.log('🚀 [SCANNER-PRIORITY] Events B >= 20! Activating arbitrage engine priority.');
            this.gateway.sendUpdate('system_log', {
                level: 'info',
                message: `🚀 SCANNER PRIORITY ACTIVE: Events B = ${countB}. Arbitrage engine is now prioritized!`
            });
        ...
        if (this.scannerPriorityActive && (now - this.lastPriorityBroadcast) > 5000) {
            ...
            console.log(`[SCANNER-PRIORITY] 📊 A:${countA} B:${countB} Pairs:${pairsCount} (Priority Mode ACTIVE)`);
    ...
    public resolveGlobalEventId(account: string, rawId: string): string[] {
        // 🛡️ v8.5 [STRICT]: Fail-Fast Identity Resolution (No more synthetic aliases)
    ...
    public async registerMatch(account: 'A' | 'B', subProvider: string, rawData: any) {
        if (account === 'A' || account === 'B') {
            ...
            this.oipmCounts[account]++; // 🛡️ v7.4 OIPM Tracking
        }

        // 🛡️ v3.5.1 EMERGENCY: Ultra-aggressive team extraction
        ...
        // 🛡️ v7.1: Support combined 'match' field (OpticOdds style)
        ...
        // 🛡️ v8.2 ID GUARD: Detect and bypass "dummy" IDs from buggy sniffers (e.g. "0.6", "0.8")
        ...
        const docTitle = String(rawData.title || '').toLowerCase(); // 🛡️ v7.4 title hint

        // 🛡️ v7.4 CLIENT ISOLATION
        const clientId = rawData.clientId || 'default';
        const instanceKey = `${account}:${clientId}`;

        // 🛡️ v4.7 TOTAL VISIBILITY: Log every single match attempted
        ...
        if (account === 'B') {
            console.log(`\x1b[36m[DEBUG-ISPORT] 🏐 ATTEMPTING REGISTRATION: ${home} vs ${away} (Provider: ${subProvider})\x1b[0m`);
        }
        ...
        if (account === 'B' && !this.sampleLoggedB) {
            ...
            console.log(`[PAYLOAD-AUDIT-B] 🛰️ SAMPLE: ${sample}`);
            this.logToDisk(`[PAYLOAD-AUDIT-B] ${sample}`);
            this.sampleLoggedB = true;
        }

        // 🛡️ v3.4: ACCOUNT A - Apply virtual filter (loosened)
        if (account === 'A' || account === 'B') {
            const leagueL = league.toLowerCase();
            const homeL = home.toLowerCase();
            const awayL = away.toLowerCase();

            // 🛡️ v7.4.2 ULTRA-STRICT VIRTUAL GATE (Modifikasi: e- diizinkan untuk audit) (Modifikasi: e- diizinkan untuk audit) (Modifikasi: e- diizinkan untuk audit)
            ...
            const junkPatterns = [
                ...
                '(pg)', '[pg]', 'pg)', 'pg]', 'women', 'womens', 'w ', ' w', 'pg'
            ];
            const timePattern = /\b\d{1,2}:\d{2}\s*(?:AM|PM)?\b/i; // Matches 08:12PM, 12:30

            // 🛡️ v8.9: HEURISTIC VIRTUAL GATE (Fail-Open for Account B)
            ...
            if (isVirtual && !this.bypassAllFilters) {
                // 🚀 FAIL-OPEN: If it's Account B, we allow it even if it looks virtual, unless it's VERY obviously junk
                if (account === 'B') {
                    ...
                    if (!isExtremeJunk) {
                        ...
                        console.log(`\x1b[31m[DEBUG-ISPORT] ❌ REJECTED: ${home} vs ${away} (Reason: ${rejMsg})\x1b[0m`);
                        console.log(rejMsg);
                        return;
                    }
                } else {
            ...
            // 🛡️ v8.8: Removed strict isSportsContext check to prevent Catch-22 with junk filters
        }
        // 🛡️ v4.7: Bypass all filters to ensure data makes it to registry
        if (this.bypassAllFilters) {
            this.logToDisk(`[BYPASS-INGEST] ${account} | ${home} vs ${away}`);
            // Force valid for bypass mode
        } else {
            // 🛡️ v3.5.5 STRICT VALIDATION GATE
        ...
        // 🛡️ v4.5: Emergency Filter Bypass (Length check disabled)
        /*
        ...
        */
        ...
        // 🛡️ v8.1: Redis SETNX Lock (Prevent Double Processing)
        ...
        // 🛡️ v8.5 [OPTIMIZED PAIRING]: Don't re-search if already paired or if other registry is empty
        if (entry.globalId || otherRegistry.size === 0) return;

        let partner = otherRegistry.get(globalFingerprint);
        let pairScore = 1.0;

        // 🛡️ v6.3 SYMMETRIC PAIRING - Fuzzy Search Fallback
        if (!partner) {
            // Optimization: Only run fuzzy check if we have fewer than 1000 items or for specific high-value matches
            // To prevent event loop starvation during massive batches (e.g., 500+ events)
            if (otherRegistry.size > 1000) {
                // this.logger.debug('[PAIR-BYPASS] Registry too large for fuzzy search, awaiting exact match.');
            } else {
                let bestScore = 0;
                let bestMatch: MatchEntry | null = null;

                // ⚡ v9.0: Chunked Fuzzy Search (Prevent Event Loop Starvation)
                // For now, we use a simple loop but with an early exit threshold
                for (const [otherFinger, otherEntry] of otherRegistry.entries()) {
                    ...
                    // Optimization: Early exit if we found a very strong match
                    if (bestScore >= 0.95) break;
                }

                const POTENTIAL_MATCH_THRESHOLD = 0.55;
                if (bestMatch && bestScore >= POTENTIAL_MATCH_THRESHOLD) {
                    partner = bestMatch;
                    pairScore = bestScore;
                    (partner as any)._tempTier = bestScore >= 0.85 ? 'VERIFIED' : 'POTENTIAL';
                }
            }
        }
        ...
        if (partner) {
            ...
            const pairId = partner.globalId || partner.matchId;
            entry.globalId = pairId;

            if (!this.confirmedPairs.has(pairId)) {
                const successMsg = `[PAIR-SUCCESS] 🎉 [${entry.home} vs ${entry.away}] <-> [${partner.home} vs ${partner.away}] (Score: ${pairScore.toFixed(2)}, Tier: ${tier}, GlobalID: ${pairId})`;
    ...
    /**
     * 🛡️ v6.3 RECHECK ORPHANED MATCHES
    ...
    private async recheckOrphanedMatches() {
        ...
        if (unpairedB.length > 0 && this.registryA.size > 0) {
            console.log(`[PAIR-RECHECK] 🔄 Attempting to pair ${unpairedB.length} orphaned matches...`);
    ...
    private isRealEvent(raw: any, account?: string): { valid: boolean, reason?: string, canonical?: { home: string, away: string } } {
        ...
        // 🛡️ v8.8: Strip HTML tags immediately (e.g. <font>LIVE</font>)
        hRaw = hRaw.replace(/<[^>]*>/g, '').trim();
        aRaw = aRaw.replace(/<[^>]*>/g, '').trim();

        // 🛡️ v4.5 RECOVERY BYPASS: Ignore everything, return raw
        if (this.bypassAllFilters) {
        ...
        // 🛡️ v3.5.5 CSS/HTML Junk Guard (Relaxed: allow parentheses)
        ...
        // 🛡️ v4.1 DATA SANITIZATION: Strict Whitelist (Identical to Frontend)
        ...
        // 🛡️ v7.6: Expanded Keyword Junk Filter
        const junkKeywords = [
        ...
        const isSpam = (n: string) => {
            ...
            // 🛡️ v8.8: Exact word match or selective partials
        ...
        // 🛡️ v7.4: Loosened Multi-word Junk Check (Allow up to 10 words)
        ...
        if (isJunk(hClean) || isJunk(aClean) || isSpam(hClean) || isSpam(aClean)) {
            // 🛡️ v8.9: FAIL-OPEN for Account B
            if (account === 'B') {
    ...
    private cleanRegistry() {
        // 🚀 TAHAP 3: Eliminasi "Ghost Match" (HEARTBEAT CLEANUP)
        const now = Date.now();
        let purgedA = 0;
        let purgedB = 0;
        ...
        if (purgedA > 0 || purgedB > 0) {
            console.log(`[LOB-CLEANUP] 🧹 Ghost Match Removed: A=${purgedA}, B=${purgedB} (Stale > ${this.LOB_TTL_MS / 1000}s)`);
        }
    }

    public cleanAccount(account: string) {
        // [v7.5] RE-ENABLED: Full account purge
        console.log(`[DISCOVERY-PURGE] 🧹 Purging all matches for Account ${account}`);
        if (account === 'A') {
            this.registryA.clear();
        } else if (account === 'B') {
            this.registryB.clear();
    ...
    public deleteMatch(account: 'A' | 'B', matchId: string) {
        ...
        if (registry.delete(matchId)) {
            console.log(`[LOB-ENGINE] 🗑️ Explicit DELETE for ${account}:${matchId}`);
        }
    }
    ...
    private printStabilityReport() {
        ...
        // 🛡️ v3.5 Broadcast active events to dashboard
        try {
        ...
        if (countB > 50) {
            const flowMsg = `[STABILITY-REPORT] 🌊 ISPORT data flow is heavy (${countB} events). Iframe is flowing!`;
            console.log(flowMsg);
            this.logToDisk(flowMsg);
        }

        // 🛡️ v5.2 SCANNER PRIORITY CHECK
        ...
        if (countB < 5) {
            ...
            // 🛡️ v4.1 DATA PULSE: Alert dashboard every 20s if low
            ...
            // 🛡️ v5.2 PASSIVE MODE: NO AUTO-CLICK
            if (this.lowEventsBCounter >= 6 && !isPassiveMode) { // 6 * 10s = 60s
                this.logger.warn('⚠️ [AUTO-REFRESH] Events B < 5 for 60s. Forcing "Football" navigation via command.');
                ...
                this.logger.warn('⏸️ [PASSIVE-MODE] Auto-navigation BLOCKED. Waiting for manual navigation on Account B.');
                this.gateway.sendUpdate('system_log', {
                    level: 'info',
                    message: '⏸️ PASSIVE MODE: Auto-navigation disabled. Please navigate to Soccer manually in Account B browser.'
                });
